...
File myFile;
AccelStepper stepper_x(AccelStepper::HALF4WIRE, 8, 6, 7, 9);
AccelStepper stepper_y(AccelStepper::HALF4WIRE, 4, 2, 3, 5);
AccelStepper stepper_z(AccelStepper::HALF4WIRE, A2, A0, A1, A3);
...
int x_y_speed0 = 800; //veloce era 500
...
void loop() {
  if (Serial.available()) {
    ...
    if(a == 9) {
      if (plane_probing) {
        plane_probing = false;
        Serial.println(F("Prob:off"));
      }
      else {
        plane_probing = true;
        Serial.println(F("Prob:on"));
      }
    } else
    if(a == 10) {
      ready = true;
...
void update_limit() {
  ...
  if (401 <= mean && mean < 450) { //100 408
    limit_z = true;
    return;
  }
  if (450 <= mean && mean <= 500) { //001 480
    limit_x = true;
    return;
  }
  if (501 <= mean && mean < 580) { //101 640
    limit_x = true;
    limit_z = true;
    return;
  }
...
void home_z2() {
  ...
  delay(10);
  stepper_z.setSpeed(-z_speed0);
  while((analogRead(A7) >= 700) && ((stepper_z.currentPosition()+offset_z) > min_z*steps_per_mm_z)) {
    stepper_z.runSpeed();
  }
  stepper_z.stop();
  stepper_z.setSpeed(z_speed0);
...
//        Serial.println(strikes);
        ...
        if(alt) {
          alt = false;
          Serial.println(F("Milling alted"));
          myFile.close();
          return;
        }
      }
...
void find_boundaries(char* name) { //finds the biggest X and Y values in the file
  if (!SD.begin(10)) {
